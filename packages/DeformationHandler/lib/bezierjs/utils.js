...
function utils_lib()
{
	...
	this.utils = {
		...
		length: function( curve )
		{
			...
			var len = this.Tvalues.length;
			var i, t;

			for (i = 0; i < len; i++)
			{
				t = z * this.Tvalues[i] + z;
				sum += this.Cvalues[i] * this.arcfn(t, curve);
			}
			return z * sum;
		},
		...
		{
			var bezierJs = require( "./packages/Pencil2/lib/bezierjs/bezier.js" );			
		...
		{
			var dpoints = this.derive(points);
			var d1 = dpoints[0];
			var d2 = dpoints[1];
			...
			var d = this.compute(t, d1);
			var dd = this.compute(t, d2);
			...
			if (!kOnly)
			{
				...
				var pk = this.curvature(t-0.001, points, _3d, true).k;
				var nk = this.curvature(t+0.001, points, _3d, true).k;
				dk = ((nk-k) + (k-pk))/2;
		...
		inflections: function(points)
		{
			...
			var p = this.align(points, { p1: points[0], p2: points.slice(-1)[0] });
			var a = p[2].x * p[1].y;
			var b = p[3].x * p[1].y;
			var c = p[1].x * p[2].y;
			var d = p[3].x * p[2].y;
			...
			if (this.approximately(v1, 0))
			{
				if (!this.approximately(v2, 0))
				{
					var t = -v3 / v2;
					if (0 <= t && t <= 1)
						return [t];
		...
		pairiteration: function(c1, c2, curveIntersectionThreshold)
		{
			...
			{
				return this.bboxoverlap(pair.left.bbox(), pair.right.bbox());
			});
			...
			pairs.forEach(function(pair)
			{
				results = results.concat(
					this.pairiteration(pair.left, pair.right, threshold)
				);
			});
		...
		getccenter: function(p1, p2, p3)
		{
			...
			var arc = this.lli8(mx1, my1, mx1n, my1n, mx2, my2, mx2n, my2n);
			var r = this.dist(arc, p1);
